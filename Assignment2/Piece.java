...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for (int i = 0; i < points.length; i++) {
			TPoint tp = new TPoint(points[i]);
			if (tp.x > width) width = tp.x;
			if (tp.y > height) height = tp.y;
			body[i] = tp;
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		if (other.width != this.width || other.height != this.height || other.body.length != this.body.length) {
			return false;
		}
		...
		for (int i = 0; i < other.body.length; i++) {
			...
			for (int j = 0; j < this.body.length; j++) {
				if (other.body[i].equals(this.body[j])) contains = true;
			}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece current = root;
		while (true) {
			Piece rotated = current.computeNextRotation();
